 ...
 */
    ...
    /**
     ...
     그냥 인티저 리턴하는 함수1
     */
    func comment_Basic1(a : Int) -> Int{
        return a
    }


    /// 그냥 인티저 리턴하는 함수2(여기는 노출되지 않음)

    /// 그냥 인티저 리턴하는 함수2
    func comment_Basic2(a : Int) -> Int{
        return a
    }
...
// MARK: - Callouts
/*
 콜 아웃이라고 불리는 특정 노테이션을 사용하면 블록으로 감싸 강조하거나 하는 등의 행위를 할 수 있다.
 */
 ...
 */
fileprivate func basic_Callouts() -> Int {
    return 0
}
...
/**
 > Tip:
 > 작성하기 어려우면 다음 단계를 따라하면 된다.
 >
 > 일단 작성 완료된 함수에 add documentation 명령 실행하여 자동완성되는 목록 생성
 >
 > (단축키 혹은 메뉴는 아래와 같다)
 > * [⌘Commend] + [⌥Option] + [/]
 ...
 >
 > 필요한 Documentation 항목을 다 채워넣었으면
 > note로 작성할 목록들을 퀵주석 블럭을 생성하여 일단 내용을 채워 넣는다.
 >
 > ```
 > 코드블럭을 쉽게 적용하는 단축키는 코드블럭을 적용할 문장 전체를 선택한 뒤
 > [Caps Lock] + [`] 키를 반복 사용(3회)해 블록 주석을 적용한다.
 > ```
 >
 > 작성한 주석을 블록으로 노트로 만들고 싶은 경우
 > 첫 열을 다중행 선택하여 노트를 적용하고 싶은 > 를 삽입한다. 다중행 선택 방법은 아래와 같다.
 > * [⌥ option] + 드래그
 > * [⌃ control] + [⇧ shift] + [↓ or ↑ 커서이동]

 > Todo:
 > 간단한 영상이나 gif 파일로 적용하는 법을 찍어서 올리기...를 하면 좋겠지만, 언제 그거까지;;
 ...
 * Tip
 ...
 작성이 다 완료되고 나면 뭔가 아쉽다. 자동 완성된 퀵헬프 주석도 블록주석 안으로 넣어주고 싶다.
 다중행 선택으로 자동완성된 퀵주석과 작성한 블록주석의 앞부분만 선택하여 지우고 최상단에 블록주석을 열어준다.
 */
fileprivate func advanced_note(){
...
/**
 ...
 코드보이스는 뭔지는 잘 모르겠다.
 */
fileprivate func basic_Format() -> Int {
    return 0
}

/**
 [nextpage]:[./CH.01 Basics.playground]
 */
